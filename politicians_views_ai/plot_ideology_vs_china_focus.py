#!/usr/bin/env python3
"""
Plot politicians' ideology scores vs US-China focus scores.
...
CHINA_FOCUS_SCORES_FILE = SCRIPT_DIR / "china_focus_scores.json"
OUTPUT_FILE = SCRIPT_DIR / "ideology_vs_china_focus_plot.png"
...
def main():
    ...
    # Load US-China focus scores
    with open(CHINA_FOCUS_SCORES_FILE, 'r') as f:
        china_focus_data = json.load(f)
    
    # Match up the data
    data = []
    for name, score_data in china_focus_data['scores'].items():
        ...
        data.append({
            'name': name,
            'ideology': ideology,
            'china_focus_score': score_data['score'],
    ...
    if democrats:
        ax.scatter(
            ...
            [d['china_focus_score'] for d in democrats],
    ...
    if republicans:
        ax.scatter(
            ...
            [d['china_focus_score'] for d in republicans],
    ...
    if independents:
        ax.scatter(
            ...
            [d['china_focus_score'] for d in independents],
    ...
    ax.set_ylabel('US-China Focus Score (0-100)', 
                  fontsize=13, color=COLORS['text_color'],
                  fontfamily='sans-serif', fontweight=500, fontproperties=semibold_prop)
    ax.set_title('Political Ideology vs US-China Focus Score', 
    ...
    mean_china_focus = sum(d['china_focus_score'] for d in data) / len(data)
    
    # # Add reference lines with brand colors
    # ax.axhline(y=mean_china_focus, color=COLORS['gray_medium'], linestyle='--', 
    ...
    top_china_focus = sorted(data, key=lambda x: x['china_focus_score'], reverse=True)[:21]
    ...
    y_range = max(d['china_focus_score'] for d in data) - min(d['china_focus_score'] for d in data)

    labeled_points = []  # Track points that have labels
    min_distance = 0.08  # Adjust this threshold as needed

    for person in top_china_focus:
        # Normalize coordinates
        norm_x = person['ideology'] / x_range
        norm_y = person['china_focus_score'] / y_range
        ...
        ax.annotate(
            name,
            (person['ideology'], person['china_focus_score']),
            xytext=(0, 8),
    ...
    print(f"  Mean US-China focus score: {mean_china_focus:.1f}")
    
    print(f"\nTop 5 Most Focused on US-China Relations:")
    for i, person in enumerate(top_china_focus[:5], 1):
        print(f"  {i}. {person['name']} ({person['party']}) - US-China focus: {person['china_focus_score']:.1f}, Ideology: {person['ideology']:.2f}")
    
    # Correlation
    import numpy as np
    ideologies = [d['ideology'] for d in data]
    china_focus_scores = [d['china_focus_score'] for d in data]
    correlation = np.corrcoef(ideologies, china_focus_scores)[0, 1]
    print(f"\nCorrelation (ideology vs US-China focus): {correlation:.3f}")
