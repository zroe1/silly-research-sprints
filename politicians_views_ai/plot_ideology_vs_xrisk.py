#!/usr/bin/env python3
"""
Plot politicians' ideology scores vs XRisk-pilled scores.
...
XRISK_SCORES_FILE = SCRIPT_DIR / "xrisk_pilled_scores.json"
OUTPUT_FILE = SCRIPT_DIR / "ideology_vs_xrisk_plot.png"
...
def main():
    ...
    # Load XRisk-pilled scores
    with open(XRISK_SCORES_FILE, 'r') as f:
        xrisk_data = json.load(f)
    
    # Match up the data
    data = []
    for name, score_data in xrisk_data['scores'].items():
    ...
    if democrats:
        ax.scatter(
            ...
            [d['xrisk_score'] for d in democrats],
    ...
    if republicans:
        ax.scatter(
            ...
            [d['xrisk_score'] for d in republicans],
    ...
    if independents:
        ax.scatter(
            ...
            [d['xrisk_score'] for d in independents],
    ...
    ax.set_ylabel('XRisk-Pilled Score (0-100)', 
                  fontsize=13, color=COLORS['text_color'],
                  fontfamily='sans-serif', fontweight=500, fontproperties=semibold_prop)
    ax.set_title('Political Ideology vs XRisk-Pilled Score', 
    ...
    mean_xrisk = sum(d['xrisk_score'] for d in data) / len(data)
    ...
    top_xrisk = sorted(data, key=lambda x: x['xrisk_score'], reverse=True)[:21]
    ...
    y_range = max(d['xrisk_score'] for d in data) - min(d['xrisk_score'] for d in data)

    labeled_points = []  # Track points that have labels
    min_distance = 0.08  # Adjust this threshold as needed

    for person in top_xrisk:
        # Normalize coordinates
        norm_x = person['ideology'] / x_range
        norm_y = person['xrisk_score'] / y_range
        ...
        ax.annotate(
            name,
            (person['ideology'], person['xrisk_score']),
            xytext=(0, 8),
    ...
    print(f"  Mean XRisk score: {mean_xrisk:.1f}")
    
    print(f"\nTop 5 Most XRisk-Pilled:")
    for i, person in enumerate(top_xrisk[:5], 1):
        print(f"  {i}. {person['name']} ({person['party']}) - XRisk: {person['xrisk_score']:.1f}, Ideology: {person['ideology']:.2f}")
    
    # Correlation
    import numpy as np
    ideologies = [d['ideology'] for d in data]
    xrisk_scores = [d['xrisk_score'] for d in data]
    correlation = np.corrcoef(ideologies, xrisk_scores)[0, 1]
    print(f"\nCorrelation (ideology vs XRisk-pilled): {correlation:.3f}")
